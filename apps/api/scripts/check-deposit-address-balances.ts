...
async function main() {
  ...
  const addresses = await prisma.userDepositAddress.findMany({
    where: { network: 'ARBITRUM_SEPOLIA', status: 'ACTIVE' },
    take: 10,
  });

  for (const address of addresses) {
    const eth = await client.getBalance({ address: address.address as `0x${string}` });
    let usdc = '0';
    if (usdcAddress) {
      try {
    ...
    if (eth > 0n || usdc !== '0') {
      console.log(
        `deposit ${address.userId.slice(0, 8)} ${address.address} ETH=${formatEther(eth)} USDC=${usdc}`,
      );
    }
  }
